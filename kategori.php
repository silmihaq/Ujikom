<?php
// Nyalakan pelaporan error untuk debugging (Hanya untuk Development)
                        ...
                        <div class="col-md-6">
                            <form action="" method="GET" class="d-flex justify-content-end">
                                <div class="input-group" style="max-width: 300px;">
                                    <input type="text" name="cari" class="form-control" placeholder="Cari kategori..." value="<?= htmlspecialchars($keyword); ?>">
                                ...
                                <?php if(mysqli_num_rows($query_kategori) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="hapus_kategori.php?id=<?= $row['id_kategori']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin hapus kategori <?= htmlspecialchars($row['nama_kategori']); ?>?')">
                    ...
                    <nav>
                        <ul class="pagination justify-content-end">
                            <li class="page-item <?= ($halaman <= 1) ? 'disabled' : ''; ?>">
                                <a class="page-link" href="?halaman=<?= $previous ?>&cari=<?= htmlspecialchars($keyword) ?>">Previous</a>
                            </li>
                            <?php for($x = 1; $x <= $total_halaman; $x++): ?>
                                <li class="page-item <?= ($halaman == $x) ? 'active' : ''; ?>">
                                    <a class="page-link" href="?halaman=<?= $x ?>&cari=<?= htmlspecialchars($keyword) ?>"><?= $x ?></a>
                                </li>
                            <?php endfor; ?>
                            <li class="page-item <?= ($halaman >= $total_halaman) ? 'disabled' : ''; ?>">
                                <a class="page-link" href="?halaman=<?= $next ?>&cari=<?= htmlspecialchars($keyword) ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
